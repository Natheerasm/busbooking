...
<main>
  <?php if (isset($_POST['search_but'])) {
    ...
    if ($dep_city === $arr_city) {
      ...
      echo '<script>alert("Select different value for departure city and arrival city") 
      window.location;</script>';
      exit();
    }
    if ($dep_city === '0') {
      header('Location: index.php?error=seldep');
      echo '<script>alert("Select different value for departure city")
       window.location.href="index.php?error=seldep";</script>';
      exit();
    }
    if ($arr_city === '0') {
      header('Location: index.php?error=selarr');
      echo '<script>alert("Select different value for arrival city")
      window.location.href="index.php?error=selarr" </script>';
      exit();
    }
  ?>
    <div class="container-md mt-2">
      <h1 class="display-4 text-center text-light">BUS FROM: <br> <?php echo $dep_city; ?>
...
<footer style="
  ...
  <em>
    <h5 class="text-light text-center p-0 brand mt-2">
      <img src="assets/images/buslogo.png" height="40px" width="40px" alt="">
      Online bus Booking- Nico Travels
    </h5>
  </em>
  <p class="text-light text-center">&copy; <?php echo date('Y'); ?> - Developed By HNDIT 2020 BATCH GROUP 04</p>
</footer>