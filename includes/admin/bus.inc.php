<?php
session_start();
if(isset($_POST['bus_but']) and isset($_SESSION['adminId'])) {
    ...
    $price = $_POST['price'];
    $air_id = $_POST['travels_name'];
    $dura = $_POST['dura'];

    if($dep_city===$arr_city || $arr_city==='To' || $arr_city==='From') {
      header('Location: ../../admin/bus.php?error=same');
      exit();
    }
    ...
    if($flag == false) {
      header('Location: ../../admin/bus.php?error=destless');
      exit();
    } else {
      $sql = "SELECT * FROM Travels WHERE travels_id =?";
      ...
      if($row = mysqli_fetch_assoc($result)) {
        $seats = $row['seats'];
        $travels_name = $row['name'];
        $sql = "INSERT INTO Bus(admin_id,arrivale,departure,Destination,source,
          travel_name,Seats,duration,Price,status,issue) VALUES (?,?,?,
          ?,?,?,?,?,?,'','')";
          
        $stmt = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt,$sql)) {
          header('Location: ../../admin/bus.php?error=sqlerr1');
          exit();          
        } else {      
          $admin_id = $_SESSION['adminId'];  
          mysqli_stmt_bind_param($stmt,'isssssisi',$admin_id,$arrival,$departure,$arr_city
            ,$dep_city,$travels_name,$seats,$dura,$price);            
          mysqli_stmt_execute($stmt); 
        }
        mysqli_stmt_close($stmt);
        mysqli_close($conn);
        header('Location: ../../admin/bus.php?flight=success');
        exit();
      } else {
        header('Location: ../../admin/bus.php?error=sqlerr');
        exit();
      }
    }
} else {
